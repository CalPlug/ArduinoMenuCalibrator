...
StatisticalCalibrator::StatisticalCalibrator(int fitSelection)
{
	...
	else if (fitSelection == 6)
	{
		...
		Serial.println("Polynomial Degree: ");   // prompt user
	...
	else if (fitSelection == 9)
	{
		...
		delete[] py;
		delete[] pyregress;
		Serial.println("Pts removed, restarting...");
	...
	else if (fitSelection == 10)
	{
		if (usecached == 0)
		{
			Serial.println("By toggling this option to 1 or 'On', fit will \nuse cache data for any subsequent fits.");
		}
		else
		{
			Serial.println("By toggling this option to 0 or 'Off', enter \npoints in on next fit.");
		}
	...
	{
		Serial.println("Invalid sel. Res. calibration...");
...
//Wattmeter sample library below (used for comparison)
